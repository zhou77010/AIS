...
# What the runtime remembers between restarts is written down rather than configured,
# but where it is written is a path like any other.
DEFAULT_STATE_RELATIVE_PATH = Path("state") / "runtime.json"
...
class Config:
    ...
    state_file: Path = PROJECT_ROOT / DEFAULT_STATE_RELATIVE_PATH
...
def _resolve_state_file() -> Path:
    """Return the runtime state file, resolved against the root."""
    raw = _read_value(EnvVar.STATE_FILE)
    if raw is None:
        return PROJECT_ROOT / DEFAULT_STATE_RELATIVE_PATH
